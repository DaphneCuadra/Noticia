...
def periodistadetalle(request, id):
    response = requests.get(f'http://127.0.0.1:8000/api/periodistas/{id}/') 
...
def index (request):
    ...
    response2 = requests.get('https://www.mindicador.cl/api') 
...
def periodistasupdate (request, id):
    ...
    if request.method == 'POST':
        ...
        if  formulario.is_valid():
            ...
            aux['form'] = formulario
            messages.error('Periodista NO Modificado!')
